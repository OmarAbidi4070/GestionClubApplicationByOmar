...
const ClubManagement = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const approvedClubs = approvedClubsResponse.data.filter((club) => club.statutValidation === "validé")
        ...
        const pendingClubs = pendingClubsResponse.data.filter((club) => club.statutValidation === "en_attente")
  ...
  const handleApproveClub = async (clubId) => {
    try {
      ...
      console.log("API URL:", `http://localhost:5000/api/clubs/${clubId}/approve`)

      await axios.put(
        `http://localhost:5000/api/clubs/${clubId}/approve`,
        {},
        {
          headers: { "x-auth-token": token },
        },
  ...
  const handleRejectClub = async (clubId) => {
    try {
      ...
      console.log("API URL:", `http://localhost:5000/api/clubs/${clubId}/reject`)

      await axios.put(
        `http://localhost:5000/api/clubs/${clubId}/reject`,
        {},
        {
          headers: { "x-auth-token": token },
        },
  ...
  const filteredPendingClubs = pendingClubs.filter(
    (club) =>
      club.nom.toLowerCase().includes(searchTerm.toLowerCase()) ||
      club.description.toLowerCase().includes(searchTerm.toLowerCase()) ||
      club.etablissement.toLowerCase().includes(searchTerm.toLowerCase()),
              ...
              <div className="club-details-header">
                ...
                <div className="club-details-info">
                  ...
                  {selectedClub.statutValidation && (
                    <span
                      className={`status-badge ${selectedClub.statutValidation === "validé" ? "active" : "inactive"}`}
                    >
                      {selectedClub.statutValidation === "validé"
                        ? "Actif"
                        : selectedClub.statutValidation === "refusé"
                          ? "Refusé"
                          : "En attente"}
                    </span>
                  )}
